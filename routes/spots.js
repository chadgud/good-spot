...
const { isLoggedIn, validateSpot, isAuthor } = require('../middleware');
...
router.route('/')
    .get(catchAsync(spots.getAllSpots))
    .post(isLoggedIn, upload.array('image'), validateSpot, catchAsync(spots.createNewSpot));
router.get('/new', isLoggedIn, spots.renderNewForm);

router.route('/:id')
    .get(catchAsync(spots.getSpot))
